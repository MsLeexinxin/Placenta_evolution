...
from model.saturn_model import SATURNPretrainModel, SATURNMetricModel, make_centroids, make_hierarchical
...
def trainer(args):
    ...
    min_mean = 0.0125 #if_not_test_else(0.0125, -np.inf)
    max_mean = 3 #if_not_test_else(3, np.inf)
    min_disp = 0.5 #if_not_test_else(0.5, -np.inf)
    max_disp = None #if_not_test_else(None, np.inf)

    for species in sorted_species_names:
        adata = species_to_adata[species]
        if use_batch_labels:
            print("sc.pp.highly_variable_genes", species, adata)
            target_cells = 9999999
            adatas = [adata[adata.obs[adata_label].isin([clust])] for clust in adata.obs[adata_label].cat.categories]
            for dat in adatas:
            ...
            if args.hv_genes == 0:
                ...
                sc.pp.log1p(adata)
                sc.pp.highly_variable_genes(adata, min_mean=min_mean, min_disp=min_disp, batch_key=args.non_species_batch_col)
                adata.X = adata.layers["raw_counts"].copy()
                print("{} highly variable genes".format(adata.var["highly_variable"].sum()))
            else:
                sc.pp.highly_variable_genes(adata, flavor='seurat_v3', n_top_genes=args.hv_genes, batch_key=args.non_species_batch_col, span=args.hv_span)
        else:
            ...
            if args.hv_genes == 0:
                ...
                sc.pp.normalize_total(adata, target_sum=None)
                sc.pp.log1p(adata)
                sc.pp.highly_variable_genes(adata, min_mean=min_mean, min_disp=min_disp)
                adata.X = adata.layers["raw_counts"].copy()
                print("{} highly variable genes".format(adata.var["highly_variable"].sum()))
            else:
                sc.pp.highly_variable_genes(adata, flavor='seurat_v3', n_top_genes=args.hv_genes)
    ...
    if args.centroids_init_path is not None:
        ...
        try:
            ...
            species_genes_scores = make_hierarchical(X, all_gene_names, args.work_dir, args.num_macrogenes, seed=args.seed, score_function=args.centroid_score_func)
        ...
        #species_genes_scores = make_centroids(X, all_gene_names, args.num_macrogenes, seed=args.seed, score_function=args.centroid_score_func)
        species_genes_scores = make_hierarchical(X, all_gene_names, args.work_dir, args.num_macrogenes, seed=args.seed, score_function=args.centroid_score_func)
        print(len(species_genes_scores))
        print(species_genes_scores)

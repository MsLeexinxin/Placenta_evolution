...
### Directory Structure
Create a folder named `01.blastout`. Inside this folder, create subdirectories for each species pair, where the directory name consists of two-letter species codes. For example, `HOMA` represents the human (HO) and cynomolgus monkeys (MA) pair.
...
For each species pair directory, you need to generate two reciprocal BLAST alignment files. Each file should be named in the format `{species1}_to_{species2}.txt`.
...
blastp -query $seqdir/MA.pep.fa \
       -db $seqdir/HO.pep.fa \
       -outfmt 6 \
       -out $outdir/MA_to_HO.txt \
       -num_threads $cpus \
       -max_hsps 1 \
       -evalue 1e-6 >> logs/blast.MAtoHO.log 2>&1
```

**Important:** The query species comes first in the filename (e.g., `HO_to_MA.txt` indicates human proteins were used as queries against the macaca protein database).
...
- **Cluster information:** Ensure the h5ad files contain cluster labels in the `seurat_clusters` column (or other specified annotation column)

### SAMap Command
Run SAMap for each species pair using the following command structure:

```bash
python /share/home/zhanglab/user/lixin/placenta_analysis/snRNA-analysis/Samap/run_samap.py \
  -a HO \              # First species code
  -A MA \              # Second species code
  -k seurat_clusters \ # Cluster column for first species
  -K seurat_clusters \ # Cluster column for second species
  -f ./h5ad_file/HO.h5ad \     # h5ad file for first species
  -F ./h5ad_file/MA.h5ad \     # h5ad file for second species
...
```bash
python twospecies.readpkl_plot.py
# This generates: HOMA.results.txt, MAMU.results.txt, TUDO.results.txt, 
# DOOV.results.txt, OVGO.results.txt, GOSU.results.txt
```

Each text file (`{species_pair}.results.txt`) contains the SAMap mapping scores between cell types/clusters of the two species.
...
```bash
python step1.creat.merge.HO_MA_MU_TU_DO_OV_GO_SU.py
```

**Input files required:**
- `HOMA.results.txt` (Human-Macaca)
- `MAMU.results.txt` (Macaca-Mouse)
- `TUDO.results.txt` (Tree shrew-Dog)
- `DOOV.results.txt` (Dog-Sheep)
- `OVGO.results.txt` (Sheep-Goat)
- `GOSU.results.txt` (Goat-Pig)
...
```bash
python step2.sanky.HO_MA_MU_TU_DO_OV_GO_SU.py
```
...
**Note:** Ensure all required Python scripts (`twospecies.readpkl_plot.py`, `step1.creat.merge.py`, `step2.sanky.py`) are in your working directory or accessible in your PATH before running the pipeline.
